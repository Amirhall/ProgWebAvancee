...
namespace signalr.backend.Hubs
{
    ...
    public class ChatHub : Hub
    {
        ...
        public async override Task OnConnectedAsync()
        {
            ...
            var userList = UserHandler.UserConnections.ToList();
            await Clients.All.SendAsync("UsersList", userList);
            var channelList = _context.Channel.ToList();
            await Clients.All.SendAsync("channelList", channelList);
        }
        ...
        public async Task JoinChannel(int oldChannelId, int newChannelId)
        {
            ...
            await Groups.RemoveFromGroupAsync(Context.ConnectionId, oldChannelId.ToString());
            await Clients.User(CurentUser.Id).SendAsync("leaveChannel");

            // TODO: Faire joindre le nouveau canal à l'utilisateur
            await Groups.AddToGroupAsync(Context.ConnectionId, oldChannelId.ToString());
            SendMessage(userTag + " a rejoint : " + "non", newChannelId, CurentUser.Id);
        ...
        public async Task SendMessage(string message, int channelId, string userId)
        {
            ...
            else if (channelId != 0)
            {
                ...
                Channel channel = _context.Channel.Find(channelId);
                await Clients.Group(channelId.ToString()).SendAsync("NewMessage", "["+ channel.Title+ "] "+ message);
            }
            else
            {
